"""Thread-safe pyodbc connection pool for Azure SQL Database.

Identical to the MSSQL pool with Azure-specific connection string handling:
- Auto-appends ``MARS_Connection=yes`` if absent
- Validates Azure SQL connection string format
- Supports both Azure AD and SQL authentication
"""
...
class ConnectionPool:
    ...
    Works with both Azure SQL Database (``*.database.windows.net``)
    and on-premises SQL Server instances.
    """
    ...
    def is_azure(self) -> bool:
        """Return True if the connection string targets Azure SQL Database."""
        return ".database.windows.net" in self._conn_str.lower()
